...
public class EventTest {
  ...
  public void parseJsonEvent() {
    ...
    InputStream input = EventTest.class.getResourceAsStream("/request_json.json");
    ...
    try {
      ...
      Assert.assertEquals("MatrikulaerSagCreate",
          eventMessage.messageEnvelope.filterData.messageType);
      ...
      Assert.assertEquals("Peter West-Nielsen",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).registrator);
      Assert.assertNull(
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).registrationTime);
      Assert.assertEquals("Afsluttet",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).status);
      Assert.assertEquals("Geodatastyrelsen",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).objectResponsible);
      Assert.assertEquals("ID20165",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).objectId);
      Assert.assertEquals("MatrikulaerSag",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).objectType);
      Assert.assertNull(
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).objectAction);
      Assert.assertEquals("52.20.05",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).objectSubject);
      Assert.assertEquals("5,0001-01-01T00:00:00.0000000",
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).registrationId);
      Assert.assertNull(
          eventMessage.messageEnvelope.filterData.objectRegistrations.get(0).localization);
      ...
      Assert.assertEquals("Ikke fortrolige data",
          eventMessage.messageEnvelope.deliveryInformation.securityClassification);
  ...
  public void parseXmlEvent() {
    ObjectMapper mapper = new ObjectMapper();
    InputStream input = EventTest.class.getResourceAsStream("/request_xml.json");
    Assert.assertNotNull(input);
    try {
      BusinessEvent event = mapper.readValue(input, BusinessEvent.class);
      Assert.assertEquals("#DataDistributor.Event", event.eventType);
      Assert.assertEquals("XML", event.format);
      Assert.assertEquals(1, event.id);
      EventMessage eventMessage = event.getEventBody().eventMessage;
      Assert.assertEquals("1.0", eventMessage.messageVersion);
      Assert.assertNull(event.getEventBody().eventMessage.messageId);
